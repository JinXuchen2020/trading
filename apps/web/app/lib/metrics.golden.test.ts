...
describe('黄金案例验证', () => {
  ...
  test('案例A：原始综合场景 （TSLA + GOOGL）', () => {
    ...
    expectMetricsToBeClose(metrics, {
      ...
      M2: 35050, // TSLA 50*105=5,250 + GOOGL -20*1490=-29,800 = -24,550 (注意空头为负)
      M3: 700,   // TSLA (105-95)*50=500 + GOOGL (1500-1490)*20=200 = 700
      M4: 1500,  // 所有今日已实现盈亏: TSLA 1500 + GOOGL 400 = 1900
      M5: {fifo: 1900, trade: 900},   // 当日部分: TSLA (105-95)*50=500 + GOOGL (1500-1480)*20=400 = 900
      M6: 2200,  // M3 + M4 = 700 + 1900 = 2,600
      M7: {B: 1, C: 1, P: 1, S: 1, total: 4},     // 今日4笔: TSLA的B1 S1, GOOGL的P1 C1
      M8: {B: 2, C: 1, P: 1, S: 1, total: 5},     // 历史1 (TSLA的B) + 今日4 = 5
      M9: 0,     // 历史已实现盈亏 (不含今日)
      M10: {L: 0, W: 2, rate: 100},  // 胜率100% (2/2)
  ...
  test('案例B：纯多头 FIFO 出清', () => {
    ...
    expectMetricsToBeClose(metrics, {
      ...
      M5: {"fifo": 2250, "trade": 0},     // 无日内交易
      M6: 2500,  // M3 + M4 = 250 + 2250 = 2,500
      M7: {"B": 0, "C": 0, "P": 0, "S": 1, "total": 1},     // 今日1笔: S1
      M8: {"B": 2, "C": 0, "P": 0, "S": 1, "total": 3},     // 历史2 (B2) + 今日1 (S1) = 3
      M9: 0,     // 历史已实现盈亏(不含今日) = 0
      M10: {"L": 0, "W": 1, "rate": 100},  // 胜率100% (1/1)
  ...
  test('案例C：纯空头 FIFO 回补', () => {
    ...
    expectMetricsToBeClose(metrics, {
      ...
      M5: {"fifo": 1800, "trade": 0},     // 无日内交易
      M6: 2400,  // M3 + M4 = 600 + 1800 = 2,400
      M7: {"B": 0, "C": 1, "P": 0, "S": 0, "total": 1},     // 今日1笔: C1
      M8: {"B": 0, "C": 1, "P": 1, "S": 0, "total": 2},     // 历史1 (P) + 今日1 (C1) = 2
      M9: 0,     // 历史已实现盈亏(不含今日) = 0
      M10: {"L": 0, "W": 1, "rate": 100},  // 胜率100% (1/1)
  ...
  test('案例D：同日部分成交 + 再建仓', () => {
    ...
    expectMetricsToBeClose(metrics, {
      ...
      M4: 0,      // 当日已实现盈亏 (22-20)*30 = 60
      M5: {"fifo": 60, "trade": 60},      // 日内交易 (22-20)*30 = 60
      M6: 290.4,   // M3 + M4 = 290.4 + 60 = 350.4
      M7: {"B": 2, "C": 0, "P": 0, "S": 1, "total": 3},       // 今日3笔: B2 S1
      M8: {"B": 2, "C": 0, "P": 0, "S": 1, "total": 3},       // 无历史，今日3笔: B2 S1
